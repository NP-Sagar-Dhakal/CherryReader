...
package com.cherry.reader.ui.main
...
import com.cherry.reader.App
import com.cherry.reader.data.entities.Feed
import com.cherry.reader.data.entities.FeedWithCount
import com.cherry.reader.data.utils.PrefConstants
import com.cherry.reader.service.AutoRefreshJobService
import com.cherry.reader.service.FetcherService
import com.cherry.reader.ui.discover.DiscoverActivity
import com.cherry.reader.ui.entries.EntriesFragment
import com.cherry.reader.ui.entrydetails.EntryDetailsActivity
import com.cherry.reader.ui.entrydetails.EntryDetailsFragment
import com.cherry.reader.ui.feeds.FeedAdapter
import com.cherry.reader.ui.feeds.FeedGroup
import com.cherry.reader.ui.feeds.FeedListEditActivity
import com.cherry.reader.utils.*
...
class MainActivity : AppCompatActivity(), MainNavigator, AnkoLogger {
    ...
    companion object {
        ...
        private const val INTENT_UNREADS = "com.cherry.reader.intent.UNREADS"
        private const val INTENT_ALL = "com.cherry.reader.intent.ALL"
        private const val INTENT_FAVORITES = "com.cherry.reader.intent.FAVORITES"
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        App.db.feedDao().observeAllWithCount.observe(this@MainActivity, { nullableFeeds ->
            nullableFeeds?.let { feeds ->
                ...
                newFeedGroups.addAll(
                    subFeedMap[null]?.map { FeedGroup(it, subFeedMap[it.feed.id].orEmpty()) }
                        .orEmpty()
                )
                ...
                if (hasFeedGroupsChanged(feedGroups, newFeedGroups)) {
                    ...
                    if (hasFetchingError()) {
                        ...
                        drawer_hint.textColor = ContextCompat.getColor(this, R.color.normal_text)
                    ...
                    PopupMenu(this, view).apply {
                        ...
                        when {
                            ...
                            feedWithCount.feed.retrieveFullText -> menu.findItem(R.id.enable_full_text_retrieval).isVisible =
                                false
                            else -> menu.findItem(R.id.disable_full_text_retrieval).isVisible =
                                false
                        }
        ...
        if (getPrefBoolean(PrefConstants.REFRESH_ON_STARTUP, defValue = false)) {
    ...
    override fun onStart() {
        ...
        if (getPrefBoolean(PrefConstants.HIDE_NAVIGATION_ON_SCROLL, true)) {
        ...
        doAsync {
            try {
                ...
                try {
                    ...
                    val content =
                        BufferedInputStream(contentResolver.openInputStream(uri)!!).bufferedReader()
                            .use { it.readText() }
